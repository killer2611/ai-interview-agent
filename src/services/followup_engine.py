...
class FollowUpEngine:
    ...
    def decide(
        ...
        if strength == AnswerStrength.STRONG:
            if followup_count < max_followups:
                ...
                return FollowUpDecision(
                    ...
                    followup_context=f"Candidate demonstrated mastery of {concepts}. Ask a deeper question on architecture, edge cases, or tradeoffs.",
                    adjust_difficulty=1,
                )
            else:
        ...
        if strength == AnswerStrength.PARTIAL:
            if followup_count < max_followups:
                missing = ", ".join(evaluation.missing_concepts[:2]) if evaluation.missing_concepts else "missing details"
                return FollowUpDecision(
                    action=FollowUpAction.FOLLOW_UP,
                    rationale=f"Partial answer — probing missing concept: {missing}",
                    followup_context=f"Probe the specific missing aspect: {missing}.",
                    adjust_difficulty=0,
                )
            else:
        ...
        if strength == AnswerStrength.WEAK:
            if followup_count < 1:
                return FollowUpDecision(
                    ...
                    followup_context="Simplify the question. Test foundational prerequisite understanding.",
                    adjust_difficulty=-1,
                )
            else:
